...
**개선 필요한 코드:**
```typescript
const expensiveResult = heavyCalculation(data); // 매 렌더링마다 재실행

const userInfo = await fetchUser(userId);   // 순차 실행
const repoInfo = await fetchRepo(repoId);   // 불필요한 대기 발생
```
→ 메모이제이션 미적용, 병렬화 가능한 비동기 작업 순차 실행
